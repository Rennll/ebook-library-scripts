...
// @version      1.0.0
// @description  匯出 PUBU 電子書書單為標準 JSON 格式
// @author       Re
// @match        https://www.pubu.com.tw/bookshelf*
// @grant        none
// @downloadURL  https://raw.githubusercontent.com/Rennll/ebook-library-scripts/main/pubu.user.js
// @updateURL    https://raw.githubusercontent.com/Rennll/ebook-library-scripts/main/pubu.user.js
// ==/UserScript==
  ...
  async function run() {
    ...
    const parseHtml = (doc) => {
      doc.querySelectorAll('article.col').forEach(article => {
        ...
        const title = article.querySelector('h3 a')?.getAttribute('title')?.trim()
        ...
        books.push({
          ...
          url: `https://www.pubu.com.tw/ebook/${productId}`,
        });
      });
    };
    ...
    console.log(`[PUBU 匯出] 共 ${totalPages} 頁`);
    parseHtml(firstDoc);

    for (let page = 2; page <= totalPages; page++) {
      btn.textContent = `⏳ 第 ${page} / ${totalPages} 頁...`;
      console.log(`[PUBU 匯出] 抓取第 ${page} 頁...`);
    ...
    a.download = `pubu_library_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();

    console.log(`[PUBU 匯出] 完成！共 ${books.length} 本書。`);
